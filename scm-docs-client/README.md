# راهنمای استفاده از `scm-docs-client`

## هدف

ماژول `scm-docs-client` یک کتابخانه‌ی مشترک برای انتشار مستندات داخلی SCM است. هر ماژول می‌تواند با اضافه‌کردن این dependency، مستندات خود را از طریق endpointهای استاندارد `/docs` و `/docs/api` منتشر کند.

این مستند توضیح می‌دهد:

1. `scm-docs-client` چه کاری انجام می‌دهد.
...
`scm-docs-client` سه نقش اصلی دارد:
...
```gradle
dependencies {
    implementation project(':scm-docs-client')
}
```

اگر بعداً `scm-docs-client` به Maven repository داخلی publish شود:

```gradle
dependencies {
    implementation 'ir.daneshrefah.scm.docs:scm-docs-client:1.0.0-SNAPSHOT'
}
```
...
`scm-docs-client` از properties با prefix زیر استفاده می‌کند:
...
3. ماژول `scm-docs-client` dependency شده باشد.
