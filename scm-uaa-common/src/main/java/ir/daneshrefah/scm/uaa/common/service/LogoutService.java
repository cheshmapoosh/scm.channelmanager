...
public class LogoutService {
    ...
    public void sendLogoutMessage(Authentication authentication) {
        try {
            if (authentication != null && authentication.isAuthenticated() && properties.getEnabled()) {
                ...
                sendLogoutMessage(user.getNickname(), user.getTerminalCode());
            }
        } catch (Exception e) {
    ...
    @Async
    public void sendLogoutMessage(String username, String terminalCode) {
        try {
            String value = username + DOUBLE_COLON + terminalCode;
