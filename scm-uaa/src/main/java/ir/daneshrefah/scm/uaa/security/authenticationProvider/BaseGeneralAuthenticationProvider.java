...
public abstract class BaseGeneralAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        UserLoginAuthenticationToken authorization = null;
        try {
            authorization = (UserLoginAuthenticationToken) authenticationManager.authenticate(token);
        } catch (Exception e) {
            log.error("authenticate is failed {}", token, e);
            throwError(token, e);
        }
        Exception exception = authorization instanceof AuthenticationOutcomeToken outcomeToken
                ? outcomeToken.getException()
                : null;
