...
public class ServiceComponentPoolGenerator extends RouteBuilder {
    ...
    private AbstractServiceComponentProvider serviceComponentProvider;
    private List<ServiceComponent> serviceComponents;
    private ErrorMappingService errorMappingService;

    public ServiceComponentPoolGenerator(AbstractServiceComponentProvider serviceComponentProvider,
    ...
    public void configure() {
        for (Iterator<ServiceComponent> iterator = serviceComponents.iterator(); iterator.hasNext(); ) {
            ...
            String fromUri = "SVC_" + serviceComponentProvider.getServiceComponentProvider().getCode() + "_" + serviceComponent.getCode();
...
//                        .to(toUri)
                        .process(exchange -> {
                            serviceComponentProvider.execute(exchange.getMessage().getBody(Message.class), serviceComponent);
                        })
//                        .process(exchange -> {
