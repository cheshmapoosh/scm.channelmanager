...
`provider.request` is recorded after rate limiting, when the single transport attempt begins on an already-published session. A disconnected request fails admission before an actual transport attempt and creates no pair. Once a request event is emitted, the mandatory trace-aware transport lifecycle emits exactly one response event. A failed send records its failure `provider.response`; the financial request is never queued or sent again. The successful attempt is completed only after response receipt, after-receive customizers, and ISO conversion. A unified finalization path therefore retains one failure response event for response timeouts, connection loss, decoding/validation failures, and customizer failures without swallowing the original exception.
...
Connection-worker lifecycle uses the stable `SHETAB_CONNECTION_*`,
`SHETAB_RECEIVE_IDLE_TIMEOUT`, and `SHETAB_ENDPOINT_LEASE_LOST` log events documented in
`README.md`. Worker activity has no invented request correlation ID. Failure messages are
sanitized and truncated, repeated identical failures are summarized, and lifecycle logs never
contain an ISO payload or stack trace.
